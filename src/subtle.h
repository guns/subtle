...
#define SUB_STATE_TILED					(1L << 19)									///< Tiled window
...
typedef struct subsubtle_t /* {{{ */
{
	...
	struct
	{
		Window						win, views, sublets;									///< Subtle bar windows
	} bar;

	struct
	{
		unsigned long			font, border, norm, focus, 
											cover, bg;														///< Subtle colors
	} colors;
	struct
	{
		GC								font, border, invert;									///< Subtle graphic contexts
	} gcs;
	struct
	{
		Cursor						square, move, arrow, horz, vert, 
											resize;																///< Subtle cursors
	} cursors;
} SubSubtle; /* }}} */
