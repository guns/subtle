...
#define SUB_WIN_STATE_STACK		(1L << 8)							// Stacked tiling window
#define SUB_WIN_STATE_TRANS		(1L << 9)							// Transient window
#define SUB_WIN_STATE_DEAD		(1L << 10)						// Dead window

#define SUB_WIN_PREF_INPUT		(1L << 11)						// Active/passive focus-model
#define SUB_WIN_PREF_FOCUS		(1L << 12)						// Send focus message
#define SUB_WIN_PREF_CLOSE		(1L << 13)						// Send close message

#define SUB_WIN_TILE_VERT			(1L << 14)						// Vert tile
#define SUB_WIN_TILE_HORZ			(1L << 15)						// Horiz tile>
...
void subWinUnlink(SubWin *w);														// Cut window
...
SubWin *subTileNew(int mode);													// Create new tile
...
void subClientDrag(int mode, SubWin *w);						// Move/Resize a window
void subClientToggle(int type, SubWin *w);					// Toggle various states
...
#define subUtilLogDebug(...)	subUtilLog(0, __FILE__, __func__, __LINE__, __VA_ARGS__);
#else
#define subUtilLogDebug(...)
#endif /* DEBUG */

#define subUtilLogError(...)	subUtilLog(1, __FILE__,__func__, __LINE__, __VA_ARGS__);
#define subUtilLogWarn(...)		subUtilLog(2, __FILE__,__func__, __LINE__, __VA_ARGS__);

void subUtilLog(int type, const char *file, 				// Print messages
