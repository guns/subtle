...
#define SUB_ACTION_MAX     4   ///< Action max
  ...
  char *argv[])
{
  ...
  while((c = getopt_long(argc, argv, "cstvakfoFLSjlTUGugDd:hrV", long_options, NULL)) != -1)
    {
      switch(c)
        {
          /* Groups */
          ...
          case 'M': action = SUB_ACTION_MAX;     break;
          case 'F': action = SUB_ACTION_FLOAT;   break;
